...
Homebase uses real local probes and persisted evidence for DNS, Homebridge, Tailscale, OpenClaw, service logs, macOS/system logs, public route exposure, WAN latency, and Mac mini vitals. Eufy lock state is currently ignored because the Homebridge plugin source is not trusted for lock truth.
