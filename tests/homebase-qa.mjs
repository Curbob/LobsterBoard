...
function askMentionsFirstAction(answer, firstAction) {
  ...
  if (/system logs/.test(action) && /system logs/.test(text)) return true;
  return false;
}
...
function verifyReplayFixtures() {
  ...
  for (const [name, [zone, nowText]] of Object.entries(expected)) {
    ...
    if (name === 'post-outage-homebridge-ui-patch') {
      assert(replayData.services?.homebridge?.state === 'ok', 'post-outage Homebridge UI patch should keep Homebridge service healthy');
      assert(replayData.intelligence?.homebridge?.version?.state === 'info', 'post-outage Homebridge UI patch should stay maintenance-only');
      assert(replayData.houseState?.tone === 'steady', 'post-outage Homebridge UI patch should stay steady');
      assert(Array.isArray(replayData.needsDan) && replayData.needsDan.length === 0, 'post-outage Homebridge UI patch should not create review items');
      assert(replayData.dailyDecision?.slots?.[2]?.text === 'Homebridge UI has a patch update when convenient.', 'post-outage Homebridge UI patch should land in Later maintenance');
      assert(!/homebridge first|automations first|update available|optional UI|panic|watchdog|restart|reboot/i.test(JSON.stringify({
        needsDan: replayData.needsDan,
        headline: replayData.houseState?.headline,
        summary: replayData.houseState?.summary,
        primaryAction: replayData.houseState?.primaryAction,
      ...
      })), 'post-outage Homebridge UI patch should not leak maintenance copy into Now, Watch, Review, or house-state cards');
    }
    if (name === 'post-outage-optional-app-update') {
      assert(replayData.intelligence?.softwareUpdates?.state === 'info', 'post-outage optional app update should stay maintenance-only');
      assert(replayData.houseState?.tone === 'steady', 'post-outage optional app update should stay steady');
      assert(Array.isArray(replayData.needsDan) && replayData.needsDan.length === 0, 'post-outage optional app update should not create review items');
      assert(replayData.dailyDecision?.slots?.[2]?.text === 'App updates can wait until a maintenance pass.', 'post-outage optional app update should land in Later maintenance');
      assert(!/app versions|update available|optional app|maintenance update|version check/i.test(JSON.stringify({
        needsDan: replayData.needsDan,
        headline: replayData.houseState?.headline,
        summary: replayData.houseState?.summary,
        primaryAction: replayData.houseState?.primaryAction,
    ...
    if (name === 'post-outage-tailscale-offline') {
      assert(replayData.houseState?.zones?.[0]?.id === 'network', 'post-outage Tailscale outage should lead with internet');
      assert(replayData.needsDan?.[0] === 'Tailscale: offline', 'post-outage Tailscale outage should keep Tailscale as first review item');
      assert(replayData.dailyDecision?.slots?.[0]?.text === 'Check Tailscale first.', 'post-outage Tailscale outage should name Tailscale as the first action');
      assert(!/panic|watchdog|public access first/i.test(JSON.stringify({
        headline: replayData.houseState?.headline,
        summary: replayData.houseState?.summary,
        primaryAction: replayData.houseState?.primaryAction,
        dailyDecision: replayData.dailyDecision
    ...
    if (name === 'post-outage-resource-pressure') {
      ...
      assert(replayData.needsDan?.[0] === 'CPU: elevated', 'post-outage resource pressure should keep CPU as first review item');
...
function zoneRankingCoverage(fixtureContracts) {
  ...
  const expected = [
    ...
    ['post-outage-tailscale-offline', 'network', 'Internet'],
...
function testLadderSpecCoverage() {
  ...
  const required = [
    ...
    ['live-teddy-mode-honesty', /Live Teddy bridge contract/.test(script) && /liveTeddyProofStatus/.test(script) && /askSource === 'teddy'/.test(script) && /askAgentMode === 'enabled'/.test(script) && /default-local Ask mode/i.test(text)],
...
function liveTeddyProofSpecCoverage() {
  const specDir = join(process.cwd(), 'specs', '011-homebase-live-teddy-proof');
  const files = ['spec.md', 'plan.md', 'tasks.md', 'checklists/trust.md', 'quickstart.md'];
  const text = files.map(file => readFileSync(join(specDir, file), 'utf8')).join('\n\n');
  const readme = readFileSync(join(process.cwd(), 'README.md'), 'utf8');
  const packageConfig = JSON.parse(readFileSync(join(process.cwd(), 'package.json'), 'utf8'));
  const script = readFileSync(join(process.cwd(), 'scripts', 'homebase-live-teddy-proof.mjs'), 'utf8');
  const ladder = readFileSync(join(process.cwd(), 'scripts', 'homebase-test-ladder.mjs'), 'utf8');
  const required = [
    ['spec-directory', /Homebase Live Teddy Proof Spec/.test(text)],
    ['readme-linked', /specs\/011-homebase-live-teddy-proof\/spec\.md/.test(readme)],
    ['script-registered', packageConfig.scripts?.['homebase:live-teddy-proof'] === 'node scripts/homebase-live-teddy-proof.mjs'],
    ['latest-artifact', /homebase-live-teddy-proof-latest\.json/.test(script)],
    ['approved-route', /openclaw-mac-mini\.tail02a3b6\.ts\.net:10000\/pages\/teddy-house/.test(script)],
    ['missing-is-partial', /No live Teddy proof artifact/.test(script) && /status: 'partial'/.test(script)],
    ['opt-in-run', /HOMEBASE_RUN_LIVE_TEDDY_PROOF/.test(script) && /TEDDY_HOMEBASE_ASK_AGENT/.test(script)],
    ['require-mode', /HOMEBASE_REQUIRE_LIVE_TEDDY_PROOF/.test(script)],
    ['source-teddy-required', /source must be teddy/.test(script) && /proof\?\.source !== 'teddy'/.test(script)],
    ['homebase-scope', /answer escaped Homebase scope/i.test(script) && /Axon\|pipeline\|quota/.test(script)],
    ['ladder-uses-proof', /liveTeddyProofStatus/.test(ladder) && /Live Teddy proof passed/.test(ladder)],
    ['read-only', /read-only/i.test(text)]
  ].map(([name, ok]) => ({
    name,
    ok: Boolean(ok)
...
async function main() {
  ...
  gates.push({
    ...
    detail: liveTeddyProofSpec.detail
  });
  checks.push({
    name: 'live-teddy-proof-spec',
    status: liveTeddyProofSpec.status,
